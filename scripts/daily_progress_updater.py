...
功能：
  ...
  2. 用 LLM 从消息中提取各模块的进展更新（新意图分类体系）
  3. 将进展写入 data/dashboard_data.json 的 weekly_updates 字段
  4. 同步更新 Bitable 话题表（仅写入高价值事实：重大决策/里程碑/风险）

意图分类体系（2026-04-20 重构）：
  - major_decision: 重大决策（商务选型、架构拍板等）→ 直接归档，不追问
  - milestone_fact: 里程碑/进度事实（提测、上线等）→ 与 Meegle 比对后更新看板
  - risk_blocker: 风险/阻塞（外部依赖、牌照问题等）→ 推入风险池
  - routine_task: 常规缺陷/需求 → 尝试映射到 Meegle，信息不足直接丢弃
  - personal_followup: 个人跟进 → 推送个人待办，不进项目看板（当前版本忽略）
...
# 群组列表（chat_id → 群名）
# 群名是重要的领域边界上下文，会注入到 LLM Prompt 中
CHAT_GROUPS = {
...
# 高价值意图：这些意图的话题才写入 Bitable 项目记录
HIGH_VALUE_INTENTS = {"major_decision", "milestone_fact", "risk_blocker"}
...
PROGRESS_EXTRACT_PROMPT = """你是一个AI项目秘书，负责从飞书群聊记录中提取高价值的项目事实。
...
【意图分类体系】
- major_decision: 重大决策（商务三方选型、架构拍板、供应商选择等）
- milestone_fact: 里程碑/进度事实（某功能提测、上线、完成联调等）
- risk_blocker: 风险/阻塞（外部依赖不配合、牌照问题、技术风险等）
- routine_task: 常规缺陷/需求（日常 Bug 修复、小功能调整等）
- personal_followup: 个人间的事务交接（如"@A 帮 @B 处理某事"）

【提取原则】
1. 聚焦高价值事实（major_decision / milestone_fact / risk_blocker），这些是项目记录的核心
2. routine_task 仅在信息足够完整时才提取，信息不足直接忽略
3. personal_followup 不写入项目记录，直接忽略
4. 忽略纯闲聊、表情包、打招呼
    ...
    {{
      ...
      "topics": [
        {{
          "title": "话题标题（10字以内）",
          "summary": "话题摘要（1-2句话）",
          "intent": "major_decision/milestone_fact/risk_blocker/routine_task",
...
def extract_progress_from_messages(
    ...
    user_prompt = f"""消息数量：{len(lines)} 条
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": PROGRESS_EXTRACT_PROMPT.format(
...
def write_topics_to_bitable(
    ...
    """
    将 LLM 提取的高价值话题写入 Bitable 话题表。
    仅写入 is_high_value=True 的话题（major_decision / milestone_fact / risk_blocker）。
    返回写入数量。
    """
    # 过滤：只保留高价值话题
    high_value_topics = [t for t in topics if t.get("is_high_value", False)]
    ...
    for t in high_value_topics:
        intent = t.get("intent", "routine_task")
        status_label = INTENT_TO_STATUS.get(intent, "常规任务")
    ...
    if dry_run:
        logger.info("[DRY-RUN] 将写入 %d 条高价值话题到 Bitable", len(records))
        return len(records)
    ...
    try:
        ...
        if data.get("code") == 0:
            ...
            logger.info("  ✅ 写入 %d 条高价值话题到 Bitable", count)
            return count
        else:
...
def run(hours: int = DEFAULT_HOURS, dry_run: bool = False) -> Dict:
    ...
    logger.info(
        "=== 项目进度更新 完成: 消息=%d, 有更新群=%d, 更新模块=%d, 新高价值话题=%d ===",
...
def main():
    ...
    print(f"新写入高价值话题: {summary['new_topics']}")
