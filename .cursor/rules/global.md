...
### 4.1 Webhook 服务入口（`main.py`）

`main.py` 是系统的公网 Webhook 接收层，基于 FastAPI 框架实现，通过 Railway 持续运行。

**关键路由逻辑**：

| 事件类型 | 路由目标 | 处理模块 |
| :--- | :--- | :--- |
| `im.message.receive_v1`（前端相关群组） | 前端缺陷报送流程 | `scripts/frontend_defect_reporter.py` |
| `im.message.receive_v1`（通用群组） | 多对话分离流程 | `scripts/thread_separator.py` |
| Challenge 验证 | 返回 challenge 字段 | 内置处理 |

**部署步骤**（详见 `docs/module2_buffer/deployment_guide.md`）：
1. 在 Railway 连接 `gdszyy/ai-secretary-architecture` 仓库
2. 配置必要环境变量（`LARK_APP_ID`、`DASHSCOPE_API_KEY`、`MEEGLE_TOKEN` 等）
3. 获取公网域名，在飞书开放平台配置事件订阅 URL

### 4.2 多对话分离算法（`scripts/thread_separator.py`）

实现了 `docs/module2_buffer/thread_separation_algorithm.md` 中设计的两阶段分离方案：

| 阶段 | 方法 | 输出 |
| :--- | :--- | :--- |
| 第一阶段 | 时间窗口切分 + @提及图谱 + 实体提取与匹配 | 预处理上下文（供 LLM 参考） |
| 第二阶段 | LLM 深度语义分离（含 Few-shot 示例） | 标准 ThreadEvent JSON 列表 |

**调用规范**：
- 输入：消息列表（每条含 `id`/`sender`/`content`，可选 `time`/`reply_to`）
- 输出：`high_value_threads`（可直接推入信息缓冲池）+ `review_pending_threads`（待人工审核）
- 置信度阈值：0.8（低于此值标记为 `needs_review=True`）
- 过滤规则：`casual_chat` 和 `other` 意图类型的线程不推入缓冲池
