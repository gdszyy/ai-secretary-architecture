...
class Phase3ConfirmationManager:
    ...
    def load_from_phase2(self, session_id: str) -> Optional[List[Dict[str, Any]]]:
        """
        从阶段二持久化文件加载用户选定的模块列表，避免通过上下文传递。
        返回 selected_modules 列表；文件不存在时返回 None。
        """
        file_path = os.path.join(self.state_dir, f"{session_id}_phase2_final.json")
        if not os.path.exists(file_path):
            logger.warning(f"阶段二状态文件不存在: {file_path}，请确认 session_id 正确或先完成阶段二。")
            return None
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            modules = data.get("selected_modules", [])
            logger.info(f"会话 {session_id} 已从 {file_path} 加载阶段二状态，共 {len(modules)} 个模块")
            return modules
        except Exception as e:
            logger.error(f"加载阶段二状态失败: {str(e)}")
            return None
